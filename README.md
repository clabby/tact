...
in `memory/v1.sqlite3` beside the selected `config.toml`. Its default capacity is 512 records and
256 KiB of authored content. Set `memory.max_records` and `memory.max_total_content_bytes` to
positive integers to change those local limits. They also bound the local snapshots used by explicit
push and pull commands; remote namespace capacity remains owned by the remote service.
