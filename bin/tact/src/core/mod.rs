...
const SUBAGENT_INSTRUCTIONS: &str = concat!(
    ...
    "Always declare `thinking` by task difficulty, at or below the `agent.thinking` cap. Use `low` ",
    "for straightforward work and higher effort for demanding reasoning or review when the cap ",
    "allows. The choice applies to the new child without changing the parent's effort or the cap. ",
    "Example `spawn_agent` arguments:\n```json\n",
    r#"{"role":"reviewer","task":"Review the current workspace diff for correctness.","model":"selected","#,
    r#""thinking":"low","output_schema":{"type":"object","properties":{"findings":{"type":"string"}},"#,
    r#""required":["findings"],"additionalProperties":false}}"#,
    "\n```\n",
