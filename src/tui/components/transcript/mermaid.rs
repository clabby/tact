...
//!
//! Adapted from xAI's Apache-2.0 licensed `xai-grok-markdown` renderer.
...
pub(crate) fn render(
    ...
    if src.len() > MAX_SOURCE_BYTES {
        return Some(fallback(src, styles, max_width, false));
    }
...
fn fallback(
    ...
    let limit = max_width.map(|width| width.saturating_sub(4).max(1));
    let title = chunk_line(&title, limit)
        .into_iter()
        .next()
        .unwrap_or_default();
